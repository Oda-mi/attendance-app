...
という点に疑問を感じたことをきっかけに追加実装したものです<br><br>
...
について、UX観点で違和感を感じました<br><br>

これらの点についてコーチに相談した結果、
「申請データも一覧で確認できた方が実務に近く、管理しやすい」とのフィードバックを受け、<br>
通常の Attendance と AttendanceUpdateRequest（修正申請） をまとめて一覧に表示する仕様に改善しました<br>
申請データが存在する場合は、申請内容を優先して表示することで、一覧画面上でも申請状況を直感的に把握できるようにしています<br>

**2. 申請中［承認待ち］レコードの視覚的な強調表示**<br>
上記の「勤怠データの統合表示」により、<br>
一覧画面上でも申請中の勤怠データを表示できるようになった一方で、<br>
申請状態がさらに直感的に把握できるよう、UI面での改善も行っています<br><br>

コーチから「申請中であることが一目でわかるUIにした方が良い」とのフィードバックを受け、<br>
申請中の勤怠行に背景色（薄いピンク）を適用し、申請状態が視覚的に判断しやすいように改善しています<br>

**3. ソート機能の追加**<br>
「申請一覧を確認するときに、対象日順と申請日順を切り替えられると業務で使いやすい」との<br>
フィードバックを受け、一覧の操作性向上のためソート機能を追加しています<br>
...
管理者側の修正申請承認画面について、参考UIでは<br>
存在する休憩レコードに加えて「空の休憩欄」が表示される仕様となっていました<br>

しかし、本アプリでは

- 休憩時間は存在するレコードのみを表示する仕様
...
「通常勤怠（Attendance）」と「修正申請（AttendanceUpdateRequest）」を結合して表示する仕様になっています<br>
画面上では以下のように動作します：
